defmodule ResuelveFc.PersonnelTest do
  ...
  describe "players" do
    ...
    test "list_players/0 returns all players" do
      ...
      assert Personnel.list_players() == [player]
    end

    test "get_player!/1 returns the player with given id" do
      player = player_fixture()
      assert Personnel.get_player!(player.id) == player
    end

    test "create_player/1 with valid data creates a player" do
      assert {:ok, %Player{} = player} = Personnel.create_player(@valid_attrs)
      assert player.goals == 42
      assert player.name == "some name"
    end

    test "create_player/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Personnel.create_player(@invalid_attrs)
    end

    test "update_player/2 with valid data updates the player" do
      player = player_fixture()
      assert {:ok, %Player{} = player} = Personnel.update_player(player, @update_attrs)
      assert player.goals == 43
      assert player.name == "some updated name"
    end

    test "update_player/2 with invalid data returns error changeset" do
      player = player_fixture()
      assert {:error, %Ecto.Changeset{}} = Personnel.update_player(player, @invalid_attrs)
      assert player == Personnel.get_player!(player.id)
    end

    test "delete_player/1 deletes the player" do
      player = player_fixture()
      assert {:ok, %Player{}} = Personnel.delete_player(player)
      assert_raise Ecto.NoResultsError, fn -> Personnel.get_player!(player.id) end
    end

    test "change_player/1 returns a player changeset" do
      player = player_fixture()
      assert %Ecto.Changeset{} = Personnel.change_player(player)
    end
  end
end
